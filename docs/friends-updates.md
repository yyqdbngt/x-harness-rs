...
- 发布提交 `62a4eeeaa5d5cfb4ec2faeaa3404c616f54c0844`，
  [Windows CI 33967511179](https://github.com/yyqdbngt/x-harness-rs/actions/runs/33967511179)
  完整通过。
- [friends-v0.2.1](https://github.com/yyqdbngt/x-harness-rs/releases/tag/friends-v0.2.1)
  已公开，含 0.2.0 基础包、0.2.1 当前包、签名、公钥和 SHA256SUMS。
- 从公开 Release 下载后逐一复验所有哈希、两版签名，以及公钥与本地可信备份一致；
  固定 latest 地址实际返回 0.2.1，并指向对应的不可变安装包。
- 基础版二进制的版本为 0.2.0，确认内置相同公钥和滚动更新地址。
  Authenticode 状态为 NotSigned，符合此通道不购买商业发布者证书的范围。
- 0.2.0 已在维护者本机安装到独立程序目录，安装前后的原有应用状态文件哈希一致。
- DPAPI 密码备份已实际恢复并完成签名/独立验签测试。
- 尚未代用户操作真实原生窗口的下载/重启按钮；完整的 App 内 0.2.0 → 0.2.1
  安装闭环仍需用户点击并检查升级后版本，不能把 CI 或独立下载验签当作该步骤已完成。
